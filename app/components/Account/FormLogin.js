...
export default function FormLogin() {
  ...
  const [inicio, setInicio] = useState(valoresDefault);

  const onSubmit = (data) => {
    if (
      isEmpty(inicio.email) ||
      isEmpty(inicio.password)
    ) {
      ...
      firebase
        .auth()
        .logInWithEmailAndPassword(inicio.email, inicio.password)
        .then((resp) => {
          console.log(resp);
          navigation.navigate('cuentas');
        })
    ...
    if (!validarEmail(inicio.email)) {
      console.log("el mail no es correcto");
    } else if (size(inicio.password) < 6) {
  ...
  const onChange = (e, type) => {
    setInicio({...inicio, [type]:e.nativeEvent.text});
  }
